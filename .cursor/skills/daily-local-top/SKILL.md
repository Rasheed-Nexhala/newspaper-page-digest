...
description: >-
  Aggregates all newspaper PageDigest JSON files for a given day, filters by
  scope, and produces Top 5 lists for Mangaluru, Coastal Karnataka, Karnataka,
  India, International, and Sports. Writes both Markdown and JSON under
  work/<date>/Daily_top/. Use when the user asks for "top local news", "top
  Mangaluru / Karnataka / coastal / India / international / sports news",
  "daily local top", "daily top", "combine today's digests", or names this skill.
---

# Daily Local Top

After one or more newspapers for a day have been digested (`*_PageDigest.json`), build a single daily brief of the **Top 5** stories in each bucket:
...
Read `references/output-spec.md` before writing the final files. **JSON shape is frozen** — always match that spec (six buckets, fixed keys).
...
2. **Trust `scope`, but correct clear mis-tags when ranking.** If a candidate is tagged `coastal_karnataka` but the story is plainly Kerala / Tamil Nadu / other-state (e.g. Malappuram MDMA, Thiruvananthapuram politics), **do not** put it in Coastal Top 5 — skip it for that bucket (still leave the source digest unchanged). Same for soft commercial fillers crowding out civic news. For `sports`, skip stories that clearly belong in a geo bucket (e.g. Mangaluru school sports day already covered as `mangaluru`).
...
4. **Stable JSON.** `LocalTop5_*.json` must always use the exact keys in `references/output-spec.md`. Do not rename `date_slug`, `papers_scanned`, `buckets`, `candidate_count`, `selected_count`, `items`, `rank`, or `sources`. Always emit all **six** bucket keys. Consumers depend on this.
...
The script writes candidates grouped by all six scopes, each item carrying `paper`, `edition`, `page`, `index`, `headline`, `blurb`, `kind`, `scope`.
...
For each of the six scopes independently:
...
3. **Rank by news value** for that bucket's reader — not by paper brand.

   **Local buckets** (`mangaluru` / `coastal_karnataka` / `karnataka`) — signals (highest first):
   ...
   **India** — national public-impact (Centre/policy, major incidents, courts, elections, economy-wide moves); prefer hard news over soft features.

   **International** — geopolitical / humanitarian / India-relevant world impact; prefer hard news over soft travel/culture fillers tagged international.

   **Sports** — major results, tournaments, national-team / high-profile contests; skip routine local school meets already better scoped as geo news, and skip soft lifestyle sports features.

4. Skip pure lifestyle fillers, routine notices, bank/mall openings, and opinion unless clearly the day's lead story in that bucket.
...
Follow `references/output-spec.md` exactly (frozen JSON with all six buckets). Do **not** overwrite paper folders.
...
- Eligible stories only after mis-scope filter for each of the six buckets.
...
- JSON keys match the frozen schema in `references/output-spec.md` (all six `buckets` keys present).
